...
这是一个面向 AgentSphere Agent Gateway 的 Manifest V3 扩展。它通过 Chrome `declarativeNetRequest` 动态规则，给同一 Gateway 域名下的普通 HTTPS 请求和 WebSocket 握手统一增加：

- `E2b-Sandbox-Id`
- `E2b-Sandbox-Port`
- `E2B-Traffic-Access-Token`

扩展只申请 `*.huaweicloud-agentnetwork.com` 的访问权限。Sandbox ID、端口和 token 保存在当前 Chrome Profile 的 `chrome.storage.local` 中，不会写入本目录文件，也不会同步到 Chrome 账号。
...
不要同时启用 ModHeader 中的旧规则，以免不同扩展争用同名 header。Chrome 对多个扩展修改同一个 header 的结果存在优先级关系，旧规则可能覆盖当前值。

## 验证

在 DeepSeek Harness 页面按 `F12` 打开 Network：

1. `api/remote.mux` 应出现在 WS 分类中，状态为 `101 Switching Protocols`；
